...
fn test_sha1_empty_string() {
    ...
    let result = sha1.hash("").iter().map(|b| format!("{:02x}", b)).collect::<String>();
...
fn test_sha1_abc() {
    let mut sha1 = SHA1::new();
    let result = sha1.hash("abc").iter().map(|b| format!("{:02x}", b)).collect::<String>();
...
fn test_sha1_longer_message() {
    let mut sha1 = SHA1::new();
    let result = sha1.hash("The quick brown fox jumps over the lazy dog").iter().map(|b| format!("{:02x}", b)).collect::<String>();
...
fn test_sha1_repeated_characters() {
    let mut sha1 = SHA1::new();
    let result = sha1.hash("aaaaaaaaaa").iter().map(|b| format!("{:02x}", b)).collect::<String>();
...
fn test_sha1_alphanumeric_string() {
    let mut sha1 = SHA1::new();
    let result = sha1.hash("1234567890abcdefghijklmnopqrstuvwxyz").iter().map(|b| format!("{:02x}", b)).collect::<String>();
...
fn test_sha1_non_ascii_characters() {
    let mut sha1 = SHA1::new();
    let result = sha1.hash("こんにちは世界").iter().map(|b| format!("{:02x}", b)).collect::<String>();
